    ...
    <div>
      ...
      <div className="mt-8 flex flex-col gap-5">
        {contactDetails.map((item) => (
          <div key={item.label}>
            <span className="text-[8.5px] text-gold uppercase tracking-[0.2em] font-sans block">{item.label}</span>
            <span className="text-navy text-[14px] font-sans block mt-1">{item.value}</span>
            {item.sub && <span className="text-slate text-[12px] font-sans block">{item.sub}</span>}
          </div>
        ))}
      </div>
      ...
      <div className="mt-8 h-[240px] bg-cream border border-gold/15 rounded-[2px] flex items-center justify-center">
        <span className="text-slate-light text-[12px] font-sans">Map &mdash; Honolulu, HI</span>
      </div>
